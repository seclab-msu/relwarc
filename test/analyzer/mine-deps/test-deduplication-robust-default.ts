...
const deduplicationModes = [
    [DeduplicationMode.Default, DeduplicationMode.Extended],
    [DeduplicationMode.FastDefault, DeduplicationMode.FastExtended]
];

for (const [defaultMode, extendedMode] of deduplicationModes) {
    describe(`Tests for ${defaultMode} comparison hars`, () => {
        ...
        it('hars with unknown and numeric values', async () => {
            const scripts = [
                `$.ajax({
                    url: 'example/test.php?a=123&c=789'
                });
                $.ajax({
                    url: 'example/test.php?a=456&c=UNKNOWN'
                });`
            ];
            ...
            expect(hars).toContain(jasmine.objectContaining({
                'method': 'GET',
                'url': 'http://test.com/example/test.php?a=123&c=789',
